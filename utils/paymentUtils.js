...
// Route to initialize a transaction For Monify
...
// Route to handle the redirect after payment For Monify
...
router.post('/initialize-transaction/:id', async (req, res) => {
    try {
        ...
        const response = await axios.post(`${PAYSTACK_BASE_URL}/transaction/initialize`, {
            ...
            callback_url: `${REDIRECT_URL}/${req.params.id}`,
            metadata: {
                customerName,
                paymentDescription: "Jamb Focus"
            }
        ...
        if (response.data.status) {
            res.status(200).json(response.data.data.authorization_url);
        } else {
            console.error('Paystack error response:', response.data);
            res.status(500).json({ error: response.data.message || 'An error occurred' });
        }
    } catch (error) {
        console.error('Error initializing transaction:', error.response ? error.response.data : error.message);
        res.status(500).json({ error: 'An error occurred while initializing the transaction' });
